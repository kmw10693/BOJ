    ...
    private static int bfs(Node start) {
        ...
        while(!q.isEmpty()) {
            ...
            for(int i=0; i<4; i++) {
                ...
                if(nxtX < 0 || nxtX >= h || nxtY < 0 || nxtY >= w || map[nxtX][nxtY] == '*' ||  Math.abs(now.dir - i) == 2) continue;

                if(visited[i][nxtX][nxtY] > nextMirrors) {
                    q.offer(new Node(nxtX,nxtY, i, nextMirrors));
                    visited[i][nxtX][nxtY] = nextMirrors;
                }
            }
        }
        return min;
